...
제목과 본문은 자연스러운 한국어로 작성한다. 일본어 히라가나/가타카나, 중국어 문장, 힌디어 등이 섞이면 자연스러운 한국어로 바꾼다. AI, API, URL, SUV, EV 같은 일반적인 영문 약어는 허용한다. 번역투와 반복 문장을 제거한다. [IMAGE_숫자]와 [이미지 설명: ...] 형식은 유지한다. 이미지 설명은 추상적인 표현이 아니라 실제 촬영 가능한 장면으로 고친다. 주제와 직접 관련된 사물과 장소를 구체적으로 적고 불필요한 인물은 넣지 않는다. 이미지 안의 글자, 한자, 일본어, 로고, 워터마크를 요구하지 않는다. 원래 주제와 핵심 정보는 유지하며 확인되지 않은 가격, 통계, 순위, 할인율은 추가하지 않는다. 제목과 본문은 반드시 비어 있지 않게 한다.
...
${JSON.stringify(draftData)}`;
    ...
    // 이미지 프롬프트 검수 API: 사이트 종류까지 반영해 한 장의 명확한 장면으로 다시 설계합니다.
    if (url.pathname === '/api/review-image-prompt' && request.method === 'POST') {
      try {
        ...
        let sceneRule = '주제와 직접 관련된 사물 하나 또는 같은 장소에 있는 소수의 관련 사물만 보여주는 정물 사진으로 만든다. 사람은 넣지 않는다.';
        if (site === 'calc') {
          sceneRule = '계산기, 서류, 동전, 노트북, 펜, 책상처럼 계산 주제를 직접 보여주는 사물 중심의 정물 사진으로 만든다. 사람, 얼굴, 손, 신체를 절대 넣지 않는다.';
        } else if (site === 'mingka') {
          sceneRule = '자동차, 자동차 키, 계약 서류, 대시보드처럼 자동차 이용 주제를 직접 보여주는 사물 중심의 실제 사진으로 만든다. 사람, 얼굴, 손, 신체를 절대 넣지 않는다.';
        } else if (site === 'hub') {
          sceneRule = '컴퓨터 모니터, 키보드, 스마트폰, 서버 장비처럼 AI·디지털 서비스를 직접 보여주는 기기 중심의 실제 사진으로 만든다. 사람, 얼굴, 손, 신체를 절대 넣지 않는다.';
        }
        ...
        const result = await env.AI.run('@cf/meta/llama-3.3-70b-instruct-fp8-fast', {
          messages: [
            {
              role: 'system',
              content: `너는 네이버 블로그용 이미지 프롬프트 최종 검수자다. 입력 설명을 블로그 주제에 정확히 맞는 단 하나의 실제 사진 장면으로 다시 작성한다. 결과는 영어 한 문장만 출력한다. ${sceneRule} 한 이미지에 여러 개념을 섞지 않는다. 중국풍, 일본풍, 동양화, 애니메이션, 일러스트, 캐릭터, 판타지 스타일을 사용하지 않는다. 이미지 안에 글자, 한자, 일본어, 숫자, 로고, 브랜드명, 워터마크, 간판 문구를 넣지 않는다. 추상적인 개념을 그림으로 표현하지 말고 실제 사물로 바꾼다. 한국의 현대적인 환경, 사실적인 상업 사진, 자연광, 깔끔한 구도를 사용한다. 그래프나 문서가 필요해도 읽을 수 있는 글자는 만들지 않는다.`
            },
            { role: 'user', content: `블로그 주제: ${topic}\n사이트: ${site}\n원본 이미지 설명: ${source}\n위 설명을 정확히 하나의 촬영 장면으로 재작성하라.` }
          ],
          max_tokens: 500, temperature: 0.1, top_p: 0.8
        });
    ...
    if (request.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
      ...
      const injected = html.includes('enhance.js') ? html : html.replace('</body>', '<script src="/enhance.js?v=8"></script></body>');
