...
${JSON.stringify(draftData)}`;
    ...
    // 이미지 프롬프트 검수 API: 이미지 한 장당 하나의 명확한 장면만 남깁니다.
      ...
      try {
        ...
        const result = await env.AI.run('@cf/meta/llama-3.3-70b-instruct-fp8-fast', {
          messages: [
            {
              role: 'system',
              content: '너는 네이버 블로그용 이미지 프롬프트 최종 검수자다. 입력 설명을 블로그 주제에 정확히 맞는 실제 사진용 장면 하나로 다시 작성한다. 결과는 영어 한 문장만 출력한다. 한 이미지에 핵심 주제 하나만 넣고 다른 개념을 섞지 않는다. 실제 촬영 가능한 사물과 공간을 사용한다. 사람 얼굴이나 인물 초상이 핵심이 아니면 사람을 넣지 않는다. 중국풍, 일본풍, 동양화, 애니메이션, 일러스트, 캐릭터, 판타지 스타일을 절대 사용하지 않는다. 이미지 안에 글자, 한자, 일본어, 숫자, 로고, 브랜드명, 워터마크, 간판 문구를 넣지 않는다. 추상적인 개념은 실제 사물이나 상황으로 바꾼다. 주제와 무관한 장식과 인물을 넣지 않는다. 한국의 현대적인 환경을 기본으로 하며 사실적인 사진, 자연광, 깔끔한 구도를 사용한다. 그래프나 문서가 필요하면 글자를 그리지 말고 단순한 선과 도형만 있는 자료로 표현한다.'
            },
            { role: 'user', content: `블로그 주제: ${topic}\n원본 이미지 설명: ${source}\n위 설명을 주제에 맞는 단 하나의 실제 사진 장면으로 재작성하라.` }
          ],
    ...
    if (request.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
      ...
      const injected = html.includes('enhance.js') ? html : html.replace('</body>', '<script src="/enhance.js?v=7"></script></body>');
